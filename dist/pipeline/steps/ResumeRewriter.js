...
export class ResumeRewriter extends Step {
            ...
            Object.keys(optimizedResume.technicalSkills).forEach(category => {
                ...
                if (Array.isArray(skills)) {
                    ...
                    skills.forEach(skill => {
                        const isMatch = Array.from(matchedSignals).some((signal) => skill.toLowerCase().includes(signal.toLowerCase()));
                        if (isMatch) {
                            matched.push(skill);
                        }
                        else {
    ...
    async rewriteBullet(bullet, signals, evidence, tokenUsage) {
        ...
        const allowedKeywords = signalList.filter((signal) => currentBulletLower.includes(signal.toLowerCase()));
