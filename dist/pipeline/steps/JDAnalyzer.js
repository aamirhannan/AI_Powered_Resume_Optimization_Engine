...
export class JDAnalyzer extends Step {
    ...
    async execute(context) {
        ...
        const hardSkills = Object.keys(detectedSkills).filter(k => detectedSkills[k].category !== 'soft' // Assuming ontology is mostly hard skills
        );
    ...
    classifyStrength(text, keyword) {
        ...
        if (/must|required|essential|core|strong|expert/i.test(window))
            return 'must_have';
        if (/plus|nice|preferred|bonus/i.test(window))
            return 'nice_to_have';
        return 'neutral';
    }
