...
export class JDAnalyzer extends Step {
    ...
    async execute(context: any): Promise<any> {
        ...
        const detectedSkills: Record<string, DetectedSkill> = this.extractSkillsWithOntology(jdLower);
        ...
        impliedSkills.forEach((skill: string) => {
    ...
    extractSkillsWithOntology(text: string): Record<string, DetectedSkill> {
        const detected: Record<string, DetectedSkill> = {};
    ...
    checkMatch(text: string, keyword: string): boolean {
    ...
    createSignal(text: string, keyword: string, category: string, source: string): DetectedSkill {
    ...
    classifyStrength(text: string, keyword: string): string {
        const windowSize = 50;
    ...
    extractSeniority(text: string): string[] {
