...
export class ResumeRewriter extends Step {
    ...
    async execute(context: any): Promise<any> {
        const { resume, evidence, signals } = context as { resume: Resume, evidence: any, signals: any, tokenUsage: any };
        // Deep copy
        const optimizedResume: Resume = JSON.parse(JSON.stringify(resume));

        // 1. Optimize Technical Skills (Existing Logic)
        const matchedSignals = new Set<string>(evidence.matched);
            ...
            Object.keys(optimizedResume.technicalSkills).forEach(category => {
                ...
                if (Array.isArray(skills)) {
                    ...
                    skills.forEach(skill => {
                        const isMatch = Array.from(matchedSignals).some((signal: string) =>
     ...
     */
    async rewriteBullet(bullet: string, signals: any, evidence: any, tokenUsage: any): Promise<string> {
        // A. Identify Start Pattern
        let targetVerb: string | null = null;

        const signalList = evidence.matched || [];

        // Find if a matched signal maps to a start verb
        for (const signal of signalList) {
            const signalLower = (signal as string).toLowerCase();
        ...
        const allowedKeywords = signalList.filter((signal: string) =>
    ...
    passesSafetyCheck(original: string, rewritten: string): boolean {
