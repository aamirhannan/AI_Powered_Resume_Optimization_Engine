...
export class CvWolfATSAnalyzer extends Step {
    ...
    extractAndCountKeywords(jdText, resumeText) {
        const stats = {
            technical: [],
            abilities: [],
            other: [],
        ...
        const allSemanticConcepts = new Set();
    ...
    calculateScores(stats) {
        ...
        // Normalize: If we don't have 'abilities' in JD, don't penalize?
        // sticking to simple weighted sum for now, can make dynamic if needed.
        // But for the user's SDE-1 case, technical is key.

        const finalScore = (
            (this.WEIGHTS.technical * technicalScore) +
            (this.WEIGHTS.abilities * abilitiesScore) +
            (this.WEIGHTS.other * otherScore)
        ) / (this.WEIGHTS.technical + this.WEIGHTS.abilities + this.WEIGHTS.other) * 100; // Recalculate to 100 base properly? 

        // Actually the weights sum to 1.0. So just sum them.
        const weightedScore = (
            (this.WEIGHTS.technical * technicalScore) +
            (this.WEIGHTS.abilities * abilitiesScore) +
            (this.WEIGHTS.other * otherScore)
    ...
    calculateHeuristics(resumeText, jobTitle) {
    ...
    analyzeSemanticGaps(stats, jdText) {
        const gaps = [];
