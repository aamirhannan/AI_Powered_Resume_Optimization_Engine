...
export const createPDF = async (resumeData: any): Promise<Buffer> => {
    try {
        const templatePath = path.join(__dirname, '../templates/resume-template.ejs');

        // 1. Render HTML from EJS
        const html = await ejs.renderFile(templatePath, resumeData) as string;

        // 2. Launch Puppeteer
        const browser = await puppeteer.launch({
            headless: 'new' as any, // Type definition for headless sometimes outdated or specific
